...
router.post('/', checkAuth, (req, res) => {
    ...
    UserPosts.addNew(newPost).then(result => console.log(result)).catch(err => console.log(err))
...
router.delete('/:id', checkAuth, checkPostOwner, (req, res) => {
    ...
    UserPosts.deletePost(req.params.id).then(result => console.log(result)).catch(err => console.log(err))

});

router.put('/:id', checkAuth, checkPostOwner, (req, res) => {

    UserPosts.updatePost(req.params.id, req.body).then(result => res.jsonp(result)).catch(err => res.jsonp(err))
});
...
router.get('/feed/me', checkAuth, pagination(UserPost), (req, res) => {
    ...
    UserPosts.userPosts(req.decodedUser).then(result => res.jsonp(result)).catch(err => res.jsonp(err))
})

//user posts
router.get('/feed/:username', checkAuth, (req, res) => {
    UserPosts.userPosts(req.params.username).then(result => res.jsonp(result)).catch(err => res.jsonp(err))
})
